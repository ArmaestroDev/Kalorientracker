...
import de.armando.kalorientracker.GeminiApiService
...
class ApiServiceRepository(private val apiService: GeminiApiService) {
    ...
    suspend fun fetchFoodNutrition(foodName: String, description: String): FoodNutritionInfo? {
        ...
        return jsonString?.let { apiService.parseJson<FoodNutritionInfo>(it) }
    }
    ...
    suspend fun fetchActivityCalories(activityName: String): ActivityInfo? {
        ...
        val jsonString = apiService.getApiResponse(prompt)
        return jsonString?.let { apiService.parseJson<ActivityInfo>(it) }
    }
