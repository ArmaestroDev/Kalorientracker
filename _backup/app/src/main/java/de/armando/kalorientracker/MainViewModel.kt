...
import de.armando.kalorientracker.network.ClaudeApiService
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun initializeApiService(profile: UserProfile) {
        val service = when (profile.selectedProvider) {
            AiProvider.CLAUDE -> ClaudeApiService(profile.claudeApiKey)
            AiProvider.GEMINI -> GeminiApiService(profile.apikey)
        }
        apiServiceRepository = ApiServiceRepository(service)
    }

    // NEU: Hilfsfunktion, die prüft, ob der API-Schlüssel fehlt.
    private fun isApiKeyMissing(): Boolean {
        val profile = _uiState.value.userProfile
        val isMissing = when (profile.selectedProvider) {
            AiProvider.GEMINI -> profile.apikey.isBlank()
            AiProvider.CLAUDE -> profile.claudeApiKey.isBlank()
        }

        if (isMissing) {
            _uiState.update { it.copy(isLoading = false, errorMessage = "Bitte gib zuerst deinen ${profile.selectedProvider} API-Schlüssel im Profil ein.") }
            return true
        }
        return false
    }
